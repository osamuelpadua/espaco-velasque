(function(){
  ...
  document.querySelectorAll('#faqLista .faq-item').forEach(function(item){
    var cab=item.querySelector('.faq-cab'), corpo=item.querySelector('.faq-corpo');
    cab.addEventListener('click',function(){
      var aberto=item.classList.contains('aberto');
      document.querySelectorAll('#faqLista .faq-item').forEach(function(o){
        o.classList.remove('aberto');
        o.querySelector('.faq-corpo').style.maxHeight=null;
        o.querySelector('.faq-cab').setAttribute('aria-expanded','false');
      });
      if(!aberto){
